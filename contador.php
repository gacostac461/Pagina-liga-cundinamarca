...
function visitas($con) {
	// Tiempo que se guardarÃ¡ la cookie (Por defecto 10 minutos)
	$minutos = 10;
	// ------------------
	$laFecha = getdate();
	...
	if(!isset($visitante)) {
	  ...
	  $total++;
	  $visitasDelMes++;
	  $actualizaciÃ³n = mysql_query("UPDATE estadisticas set visitas=$visitasDelMes WHERE mes=$mesActual AND anio=$anioActual",$con);
	}
	$resultadoVisitas[0] = $total; //Total de visitas en la pÃ¡gina
